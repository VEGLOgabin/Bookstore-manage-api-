...
class Backend:
    ...
    def _is_fips_enabled(self) -> bool:
        if self._lib.Cryptography_HAS_300_FIPS:
            mode = self._lib.EVP_default_properties_is_fips_enabled(self._ffi.NULL)
        else:
    ...
    def create_hash_ctx(self, algorithm: hashes.HashAlgorithm) -> hashes.HashContext:
    ...
    def _csr_is_signature_valid(self, csr: x509.CertificateSigningRequest) -> bool:
    ...
    def load_elliptic_curve_private_numbers(
        ...
        self._ec_key_set_public_key_affine_coordinates(ec_cdata, public.x, public.y)
    ...
    def load_elliptic_curve_public_numbers(
        ...
        self._ec_key_set_public_key_affine_coordinates(ec_cdata, numbers.x, numbers.y)
        evp_pkey = self._ec_cdata_to_evp_pkey(ec_cdata)
        ...
        if format is serialization.PrivateFormat.TraditionalOpenSSL:
            ...
            if encoding is serialization.Encoding.PEM:
                if key_type == self._lib.EVP_PKEY_RSA:
                    ...
                    raise ValueError("Unsupported key type for TraditionalOpenSSL")
    ...
    def generate_dh_parameters(self, generator: int, key_size: int) -> dh.DHParameters:
        if key_size < dh._MIN_MODULUS_SIZE:
            raise ValueError(
                "DH key_size must be at least {} bits".format(dh._MIN_MODULUS_SIZE)
            )
    ...
    def x25519_load_public_bytes(self, data: bytes) -> x25519.X25519PublicKey:
        ...
        self.openssl_assert(res == 1)
        res = self._lib.EVP_PKEY_set1_tls_encodedpoint(evp_pkey, data, len(data))
        self.openssl_assert(res == 1)
        return _X25519PublicKey(self, evp_pkey)
            ...
            for ca in cas:
                if isinstance(ca, PKCS12Certificate):
                    ...
                    with self._zeroed_null_terminated_buf(ca_alias) as ca_name_buf:
                        res = self._lib.X509_alias_set1(ossl_ca, ca_name_buf, -1)
                        self.openssl_assert(res == 1)
                else:
