...
class Message(object):
    ...
    def toFormMarkup(self, action_url, form_tag_attrs=None, submit_text="Continue"):
        ...
        submit = ElementTree.Element(
            "input", {"type": "submit", "value": oidutil.toUnicode(submit_text)}
        )
        form.append(submit)
...
class NamespaceMap(object):
    ...
    def addAlias(self, namespace_uri, desired_alias, implicit=False):
        ...
        # per the spec.
        assert (
            desired_alias not in OPENID_PROTOCOL_FIELDS
        ), "%r is not an allowed namespace alias" % (desired_alias,)

        # Check that desired_alias does not contain a period as per
        # the spec.
        if isinstance(desired_alias, str):
            assert "." not in desired_alias, "%r must not contain a dot" % (
                desired_alias,
            )
        ...
        if current_namespace_uri is not None and current_namespace_uri != namespace_uri:
            ...
            fmt = "Cannot map %r to alias %r. " "%r is already mapped to alias %r"
        ...
        if alias is not None and alias != desired_alias:
            fmt = "Cannot map %r to alias %r. " "It is already mapped to alias %r"
            raise KeyError(fmt % (namespace_uri, desired_alias, alias))

        assert desired_alias == NULL_NAMESPACE or type(desired_alias) in [
            str,
            str,
        ], repr(desired_alias)
