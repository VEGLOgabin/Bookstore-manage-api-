...
class DiffieHellmanSHA1ServerSession(object):
    ...
    def fromMessage(cls, message):
        ...
        if (
            dh_modulus is None
            and dh_gen is not None
            or dh_gen is None
            and dh_modulus is not None
        ):
...
class AssociateRequest(OpenIDRequest):
    ...
    def fromMessage(klass, message, op_endpoint=UNUSED):
        ...
        if message.isOpenID1():
            ...
            if session_type is None:
                raise ProtocolError(message, text="session_type missing from request")

        try:
            session_class = klass.session_classes[session_type]
        except KeyError:
            raise ProtocolError(message, "Unknown session type %r" % (session_type,))
        ...
        try:
            ...
            raise ProtocolError(
                message,
                "Error parsing %s session: %s" % (session_class.session_type, why),
            )
    ...
    def answer(self, assoc):
        ...
        response.fields.updateArgs(OPENID_NS, self.session.answer(assoc.secret))

        if not (
            self.session.session_type == "no-encryption" and self.message.isOpenID1()
        ):
            # The session type "no-encryption" did not have a name
            # in OpenID v1, it was just omitted.
            response.fields.setArg(OPENID_NS, "session_type", self.session.session_type)

        return response

    def answerUnsupported(
        self, message, preferred_association_type=None, preferred_session_type=None
    ):
        ...
        if preferred_association_type:
            response.fields.setArg(OPENID_NS, "assoc_type", preferred_association_type)

        if preferred_session_type:
            response.fields.setArg(OPENID_NS, "session_type", preferred_session_type)
    ...
    def fromMessage(klass, message, op_endpoint):
        ...
        if not message.isOpenID1():
            if self.return_to is self.trust_root is None:
                raise ProtocolError(
                    message, "openid.realm required when " + "openid.return_to absent"
                )
    ...
    def answer(self, allow, server_url=None, identity=None, claimed_id=None):
        ...
        if not server_url:
            if not self.message.isOpenID1() and not self.op_endpoint:
                ...
                raise RuntimeError(
                    ...
                    "to respond to OpenID 2.0 messages." % (self,)
                )
        ...
        if allow:
            ...
            if response_identity is not None:
                response.fields.setArg(OPENID_NS, "identity", response_identity)
                if self.message.isOpenID2():
                    response.fields.setArg(OPENID_NS, "claimed_id", response_claimed_id)
        else:
...
class OpenIDResponse(object):
    ...
    def whichEncoding(self):
        ...
        if self.request.mode in BROWSER_REQUEST_MODES:
            if (
                self.fields.getOpenIDNamespace() == OPENID2_NS
                and len(self.encodeToURL()) > OPENID1_URL_LIMIT
            ):
...
class Signatory(object):
    ...
    def verify(self, assoc_handle, message):
        ...
        try:
            ...
            logger.exception("Error in verifying %s with %s: %s" % (message, assoc, ex))
            return False
        return valid
    ...
    def sign(self, response):
        ...
        if assoc_handle:
            ...
            assoc = self.getAssociation(assoc_handle, dumb=False, checkExpiration=False)
    ...
    def encode(self, response):
        ...
        if (not isinstance(response, Exception)) and response.needsSigning():
            if not self.signatory:
                raise ValueError(
                    "Must have a store to sign this request: %s" % (response,), response
                )
...
class Server(object):
    ...
    def handleRequest(self, request):
        ...
        if handler is not None:
            ...
            raise NotImplementedError(
                "%s has no handler for a request of mode %r." % (self, request.mode)
            )
    ...
    def openid_associate(self, request):
        ...
        if self.negotiator.isAllowed(assoc_type, session_type):
            assoc = self.signatory.createAssociation(dumb=False, assoc_type=assoc_type)
            return request.answer(assoc)
        else:
            message = "Association type %r is not supported with " "session type %r" % (
                assoc_type,
                session_type,
            )
            (
            ...
            return request.answerUnsupported(
                message, preferred_assoc_type, preferred_session_type
            )
...
class ProtocolError(Exception):
    ...
    def whichEncoding(self):
        ...
        if self.hasReturnTo():
            if (
                self.openid_message.getOpenIDNamespace() == OPENID2_NS
                and len(self.encodeToURL()) > OPENID1_URL_LIMIT
            ):
                return ENCODE_HTML_FORM
            else:
...
class UntrustedReturnURL(ProtocolError):
    ...
    def __str__(self):
        return "return_to %r not under trust_root %r" % (
            self.return_to,
            self.trust_root,
        )
