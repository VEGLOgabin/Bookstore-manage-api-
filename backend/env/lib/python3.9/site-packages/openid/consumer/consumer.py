...
from openid.association import Association, default_negotiator, SessionNegotiator
...
class Consumer(object):
    ...
    def complete(self, query, current_url):
        ...
        if (
            response.status in ["success", "cancel"]
            and response.identity_url is not None
        ):

            disco = Discovery(
                self.session, response.identity_url, self.session_key_prefix
            )
...
class DiffieHellmanSHA1ConsumerSession(object):
    ...
    def extractSecret(self, response):
        dh_server_public64 = response.getArg(OPENID_NS, "dh_server_public", no_default)
...
class GenericConsumer(object):
    ...
    def _complete_error(self, message, endpoint, _):
        ...
        return FailureResponse(endpoint, error, contact=contact, reference=reference)
    ...
    def _doIdRes(self, message, endpoint, return_to):
        ...
        logger.info(
            "Received id_res response from %s using association %s"
    ...
    def _idResCheckNonce(self, message, endpoint):
        ...
        if self.store is not None and not self.store.useNonce(
    ...
    def _idResCheckSignature(self, message, server_url):
        ...
        if assoc:
            if assoc.expiresIn <= 0:
                ...
                raise ProtocolError("Association with %s expired" % (server_url,))
    ...
    def _verifyReturnToArgs(query):
        ...
        for rt_key, rt_value in parsed_args:
            try:
                ...
                if rt_value != value:
                    format = (
                        "parameter %s value %r does not match " "return_to's value %r"
                    )
    ...
    def _verifyDiscoveryResultsOpenID2(self, resp_msg, endpoint):
        ...
        to_match.server_url = resp_msg.getArg(OPENID2_NS, "op_endpoint", no_default)

        # claimed_id and identifier must both be present or both
        # be absent
        if to_match.claimed_id is None and to_match.local_id is not None:
            raise ProtocolError("openid.identity is present without openid.claimed_id")

        elif to_match.claimed_id is not None and to_match.local_id is None:
            raise ProtocolError("openid.claimed_id is present without openid.identity")
        ...
        if not endpoint:
            ...
            try:
                ...
                logger.exception(
                    "Error attempting to use stored discovery information: " + str(e)
                )
    ...
    def _verifyDiscoverySingle(self, endpoint, to_match):
        ...
        if defragged_claimed_id != endpoint.claimed_id:
            raise ProtocolError(
                ...
                "Expected %s, got %s" % (defragged_claimed_id, endpoint.claimed_id)
            )
        ...
        if to_match.server_url is None:
            assert (
                to_match.preferredNamespace() == OPENID1_NS
    ...
    def _discoverAndVerify(self, claimed_id, to_match_endpoints):
        ...
        if not services:
            raise DiscoveryFailure(
                "No OpenID information found at %s" % (claimed_id,), None
            )
    ...
    def _verifyDiscoveredServices(self, claimed_id, services, to_match_endpoints):
        ...
        for endpoint in services:
            ...
            logger.error("Discovery verification failure for %s" % (claimed_id,))
            for failure_message in failure_messages:
                logger.error(" * Endpoint mismatch: " + failure_message)

            raise DiscoveryFailure(
                "No matching endpoint found after discovering %s" % (claimed_id,), None
            )
    ...
    def _processCheckAuthResponse(self, response, server_url):
        ...
        if invalidate_handle is not None:
            logger.info('Received "invalidate_handle" from server %s' % (server_url,))
            if self.store is None:
                logger.error("Unexpectedly got invalidate_handle without " "a store!")
            else:
    ...
    def _negotiateAssociation(self, endpoint):
        ...
        try:
            assoc = self._requestAssociation(endpoint, assoc_type, session_type)
    ...
    def _extractSupportedAssociationType(self, server_error, endpoint, assoc_type):
        ...
        if (
            server_error.error_code != "unsupported-type"
            or server_error.message.isOpenID1()
        ):
            logger.error(
        ...
        logger.error(
            "Unsupported association type %s: %s"
            % (
                assoc_type,
    ...
    def _requestAssociation(self, endpoint, assoc_type, session_type):
        ...
        try:
            ...
            logger.exception(
                "Protocol error parsing response from %s: %s"
                % (endpoint.server_url, why)
            )
            return None
        else:
    ...
    def _createAssociateRequest(self, endpoint, assoc_type, session_type):
        ...
        if (
            not endpoint.compatibilityMode()
            or assoc_session.session_type != "no-encryption"
        ):
    ...
    def _getOpenID1SessionType(self, assoc_response):
        ...
        if session_type == "no-encryption":
            logger.warning('OpenID server sent "no-encryption"' "for OpenID 1.X")
...
class AuthRequest(object):
    ...
    def setAnonymous(self, is_anonymous):
        ...
        if is_anonymous and self.message.isOpenID1():
            raise ValueError(
                "OpenID 1 requests MUST include the " "identifier in the request"
            )
        else:
    ...
    def getMessage(self, realm, return_to=None, immediate=False):
        ...
        if return_to:
            ...
            raise ValueError(
                'extra "return_to" arguments were specified, '
    ...
    def getSignedNS(self, ns_uri):
        ...
        for key in msg_args.keys():
            if not self.isSigned(ns_uri, key):
                logger.info(
                    "SuccessResponse.getSignedNS: (%s, %s) not signed." % (ns_uri, key)
                )
                return None
