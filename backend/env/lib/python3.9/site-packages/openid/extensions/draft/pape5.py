...
AUTH_NONE = "http://schemas.openid.net/pape/policies/2007/06/none"
...
class Request(PAPEExtension):
    ...
    def __bool__(self):
        return bool(
            ...
            or self.max_auth_age is not None
            or self.preferred_auth_level_types
        )
    ...
    def preferredTypes(self, supported_types):
        ...
        @returntype: [str]
        """
        return list(filter(self.preferred_auth_policies.__contains__, supported_types))
...
class Response(PAPEExtension):
    ...
    nist_auth_level = property(
        _getNISTAuthLevel, doc="Backward-compatibility accessor for the NIST auth level"
    )
    ...
    def addPolicyURI(self, policy_uri):
        ...
        if policy_uri == AUTH_NONE:
            raise RuntimeError("To send no policies, do not set any on the response.")
    ...
    def parseExtensionArgs(self, args, is_openid1, strict=False):
        ...
        if len(auth_policies) > 1 and strict and AUTH_NONE in auth_policies:
            raise ValueError(
                "Got some auth policies, as well as the special "
        ...
        for (key, val) in args.items():
            if key.startswith("auth_level."):
                ...
                if uri is None:
                    if strict:
                        raise ValueError("Undefined auth level alias: %r" % (alias,))
                else:
