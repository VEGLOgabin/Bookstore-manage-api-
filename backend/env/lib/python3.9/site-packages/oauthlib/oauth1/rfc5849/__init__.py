...
class Client:
    ...
    def __repr__(self):
        ...
        attrs["resource_owner_secret"] = (
            "****" if attrs["resource_owner_secret"] else None
        )
    ...
    def get_oauth_params(self, request):
        ...
        content_type_eligible = (
            content_type and content_type.find("application/x-www-form-urlencoded") < 0
        )
        if request.body is not None and content_type_eligible:
            params.append(
                (
                    "oauth_body_hash",
                    base64.b64encode(
                        hashlib.sha1(request.body.encode("utf-8")).digest()
                    ).decode("utf-8"),
                )
            )
    ...
    def sign(self, uri, http_method="GET", body=None, headers=None, realm=None):
        ...
        #       header field set to "application/x-www-form-urlencoded".
            ...
            raise ValueError(
                ...
                "instead of {}".format(
                    content_type or "not set", CONTENT_TYPE_FORM_URLENCODED
                )
            )
